...
function nbMonths(startPriceOld, startPriceNew, savingperMonth, percentLossByMonth){
  let i=0;
  let [priceOld,priceNew]=[startPriceOld,startPriceNew]
  while(true){
    if(priceOld+(i*savingperMonth)>=priceNew){
      return [i,Math.round((priceOld+(i*savingperMonth)-priceNew))]
    }
    i++;
    if(i%2==0) percentLossByMonth+=0.5
    priceOld=priceOld*(1-percentLossByMonth*.01);
    priceNew=priceNew*(1-percentLossByMonth*.01);
  }
}
